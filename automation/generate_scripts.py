...
def load_json(json_path: Path) -> dict:
    ...
    if not json_path.exists():
        raise FileNotFoundError(
            f"[!] Error: System requires {json_path.name} to exist in the execution directory."
        )
    ...
    try:
        ...
        raise KeyError(f"[!] Error: Missing required JSON key: {exc}") from exc

    if not hooks or not mids or not ctas:
        raise ValueError("[!] Error: JSON database must include hooks, mid_sections, and calls_to_action.")
...
def compatible_mid_sections(mids: list[dict], target_niche: str) -> list[dict]:
    ...
    Falls back to all mid-sections if no direct match exists.
    """
    focus_map = {
...
def compile_script(batch_id: int, current_date: str, hook: dict, mid: dict, cta: dict) -> str:
    ...
    lines = [
        "==================================================",
        "SLOTSFREEUSA AUTOMATED DAILY MARKETING VIDEO SCRIPT",
...
def load_and_compile_scripts(json_file_path: str, output_dir: str, count: int) -> None:
    ...
    for batch_id in range(1, count + 1):
        ...
        file_name = output_path / f"slotsfreeusa_script_batch_ID{batch_id}_{current_date}.txt"
        ...
        print(f" -> Compiled Asset: {file_name.as_posix()}")

    print("[+] Batch extraction complete. Drop file texts into your voiceover software.")
